...
def fetch() -> ConnectorResult:
    ...
    frm = month_start.strftime("%d.%m.%Y")
    ...
    url = (
        ...
        f"&condition[period][from]={frm}&condition[period][to]={to}"
    ...
    try:
        ...
        if not isinstance(rows, list):
            return ConnectorResult.failed(NAME, CAT, f"Unerwartete Antwort: {str(rows)[:200]}")
        ...
        # confirmed + open ≈ Awins "approved + pending"
        provision = sum(_num(r_, "all_confirmed_provision") + _num(r_, "all_open_provision") for r_ in rows)
